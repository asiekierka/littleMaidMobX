...
//			setLocationAndAngles(posX, posY, posZ, rotationYaw, rotationPitch);
	...
	public void onUpdate() {
		...
		if (inGround) {
			speed = 0F;
			if (worldObj.getBlock(inX, inY, inZ) != inBlock || --ticksInGround <= 0) {
				// 一定時間を超えた
				// 着弾していたブロックが消えた
				setDead();
			}
		...
		Vec3 lvo = Vec3.createVectorHelper(posX, posY, posZ);
		Vec3 lvt = Vec3.createVectorHelper(
				posX + motionX, posY + motionY, posZ + motionZ);
		while (true) {
			// Block用
//			movingobjectposition = worldObj.rayTraceBlocks(lvo, lvt, true);
			movingobjectposition = worldObj.func_147447_a(lvo, lvt, false, true, false);
			// Entity用
			lvo = Vec3.createVectorHelper(posX, posY, posZ);
			lvt = Vec3.createVectorHelper(posX + motionX, posY + motionY, posZ + motionZ);
			
			if (movingobjectposition != null) {
				lvt = Vec3.createVectorHelper(
			...
			if (movingobjectposition != null) {
				if (movingobjectposition.typeOfHit == MovingObjectPosition.MovingObjectType.BLOCK
						&& worldObj.getBlock(movingobjectposition.blockX,
